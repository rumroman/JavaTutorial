...
Тестовый случай (Test case) в юнит тестировании - это часть кода, которая проверяет, что другая часть кода ( в частности
метод) работает в соответствии с определенными требованиями.

Формально описанный тестовый случай характеризуется известными входными данными и ожидаемым выводом программы, который
известен до начала выполнения теста.
...
@Test - Указывает , что данный метод (public void) может быть запущен, как тестовый случай.Здесь размещаются сами
        проверки . Кроеме того, у данной аннотации есть два параметра, expected - задает одидаемое исключение и timeout
